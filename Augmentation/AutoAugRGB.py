...
aargb_train_dir = "./data/AARGB_train"  # 저장 디렉토리 "AARGB_train"

# Augmentation 설정 (AutoAugment)
transform = transforms.Compose([
    transforms.Lambda(lambda img: img.convert("RGB")),  # 3채널 이미지로 변환
    transforms.PILToTensor(),  # PIL 이미지를 텐서로 변환 (uint8로 변환)
    AutoAugment(AutoAugmentPolicy.IMAGENET),  # AutoAugment 적용
    transforms.ToPILImage()  # 다시 PIL 이미지로 변환
])

# 저장 경로가 없으면 생성
if not os.path.exists(aargb_train_dir):
    os.makedirs(aargb_train_dir)
...
for index, row in df.iterrows():
    ...
    class_save_dir = os.path.join(aargb_train_dir, img_class)
